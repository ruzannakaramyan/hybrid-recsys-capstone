...
Task limits:
1. USER PERSONA: Write a 1-sentence profile of this user's shopping behavior and interests.
2. REASONING: Explain your ranking reasoning in 2-3 sentences based on the Persona.
3. RANKING: Provide the final ranking as a comma-separated list of ONLY the Candidate IDs (1-{len(candidate_items)}) in order of highest relevance to lowest relevance.

Provide exactly this format:
PERSONA: <1-sentence profile>
...
def evaluate_api_reranker(base_model, dataset, device, k=10, rerank_topk=10, max_samples=None):
    ...
    if api_rankings:
        print(f"[PROFILE LLM] Hit@{k}: {hit_rerank/total:.8f} | NDCG@{k}: {ndcg_rerank/total:.8f}")
    print("==================================================")
